...
class ArticleTests: XCTestCase {
    ...
    func testExpandLinkTag() {
        ...
        let expandedString = self.article.expandTags(string, withConditional: true)
    ...
    func testURLIDNinArticleView() {
        let view = ArticleView(frame: NSRect(x: 0, y: 0, width: 100, height: 100))

        self.article.link = "http://ουτοπία.δπθ.gr/نجيب_محفوظ/"
        let htmlTextFromIDNALink = view.articleText(from: [ self.article ])
        ...
        let htmlTextFromResolvedIDNALink = view.articleText(from: [ self.article ])
